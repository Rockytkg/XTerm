...
pub(crate) trait Select {
    ...
    fn read_kex(
        ...
        Vec::<u8>::decode(&mut r)?; // cipher server-to-client（PATCH(xterm): 丢弃字段按字节读取，容忍非 UTF-8 内容）
        ...
        // PATCH(xterm): languages 字段仅作信息用途且值被丢弃，旧设备可能填入
        // 非 UTF-8 字节；按字节串读取跳过，避免严格 UTF-8 校验中止握手。
        Vec::<u8>::decode(&mut r)?; // languages client-to-server
        Vec::<u8>::decode(&mut r)?; // languages server-to-client
