...
impl Session {
    ...
    fn process_disconnect(
        ...
        // PATCH(xterm): 旧设备的断开原因可能是 GBK 等非 UTF-8 编码，
        // 有损解码避免误报 "character encoding invalid" 并保留可读内容。
        let message = String::from_utf8_lossy(&Vec::<u8>::decode(&mut r)?).into_owned();
        let lang_tag = String::from_utf8_lossy(&Vec::<u8>::decode(&mut r)?).into_owned();
