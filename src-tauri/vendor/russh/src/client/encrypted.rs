...
// 有损解码，避免严格 UTF-8 校验以 "character encoding invalid" 中止会话。
fn decode_string_lossy(r: &mut impl Reader) -> Result<String, ssh_encoding::Error> {
    Ok(String::from_utf8_lossy(&Vec::<u8>::decode(r)?).into_owned())
}
...
impl Session {
                ...
                EncryptedState::WaitingAuthRequest(ref mut auth_request) => {
                    ...
                    match buf.split_first() {
                        ...
                        Some((&msg::USERAUTH_BANNER, mut r)) => {
                            // PATCH(xterm): 旧设备 banner 常为 GBK 等非 UTF-8 编码，有损解码。
                            let banner = map_err!(decode_string_lossy(&mut r))?;
                            let _language_tag = map_err!(Bytes::decode(&mut r))?;
                            map_err!(ensure_end(&r))?;
                        ...
                        Some((&msg::USERAUTH_INFO_REQUEST_OR_USERAUTH_PK_OK, mut r)) => {
                            ...
                            }) = auth_request.current
                            {
                                ...
                                let _algo = map_err!(decode_string_lossy(&mut r))?;
                            ...
                            }) = auth_request.current
                            {
                                ...
                                // read fields
                                // PATCH(xterm): 旧设备 keyboard-interactive 的
                                // name/instructions/prompt 可能有非 UTF-8 文本，有损解码。
                                let name = map_err!(decode_string_lossy(&mut r))?;

                                let instructions = map_err!(decode_string_lossy(&mut r))?;

                                let _lang = map_err!(Bytes::decode(&mut r))?;
                                ...
                                for _i in 0..n_prompts {
                                    let prompt = map_err!(decode_string_lossy(&mut r))?;
    ...
    fn handle_ext_info(&mut self, r: &mut impl Reader) -> Result<(), Error> {
        ...
        for _ in 0..n_extensions {
            let name = decode_string_lossy(r)?;
    ...
    async fn client_read_authenticated<H: Handler>(
        ...
        match buf.split_first() {
            ...
            Some((&msg::CHANNEL_OPEN_FAILURE, mut r)) => {
                ...
                let descr = map_err!(decode_string_lossy(&mut r))?;
                let language = map_err!(decode_string_lossy(&mut r))?;
                map_err!(ensure_end(&r))?;
                if let Some(ref mut enc) = self.common.encrypted {
            ...
            Some((&msg::CHANNEL_REQUEST, mut r)) => {
                let channel_num = map_err!(ChannelId::decode(&mut r))?;
                let req = map_err!(decode_string_lossy(&mut r))?;
                ...
                match req.as_str() {
                    ...
                    "exit-signal" => {
                        map_err!(u8::decode(&mut r))?; // should be 0.
                        let signal_name =
                            Sig::from_name(&map_err!(decode_string_lossy(&mut r))?);
                        let core_dumped = map_err!(u8::decode(&mut r))? != 0;
                        let error_message = map_err!(decode_string_lossy(&mut r))?;
                        let lang_tag = map_err!(decode_string_lossy(&mut r))?;
                        map_err!(ensure_end(&r))?;
                        if let Some(chan) = self.channels.get(&channel_num) {
                            let _ = chan
